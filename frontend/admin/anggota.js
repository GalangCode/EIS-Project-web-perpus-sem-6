...
import { escapeHtml, renderDocument, stat, dataTable, renderPagination } from "../shared/components.js?v=20260729";
...
function buildTable(items) {
  return dataTable(
    [
      { text: "NO", width: "64px" },
      { text: "KODE", width: "110px" },
      { text: "NAMA LENGKAP", width: "180px" },
      { text: "TANGGAL LAHIR", width: "120px" },
      { text: "USIA", width: "60px" },
      { text: "JENIS KELAMIN", width: "120px" },
      { text: "NOMOR TELEPON", width: "140px" },
      { text: "STATUS", width: "96px" }
    ],
    items.map((item, index) => [
      ...
      ageLabel(item.birth_date),
      genderLabel(item.gender),
      item.phone || "-",
      item.status
    ]),
    {
      emptyText: "Tidak ada data anggota.",
      actionsWidth: "104px",
      actions: (row, index) => {
  ...
  const panelContent = state.loading
    ...
    : state.error
      ...
      : `<section class="panel" id="member-panel">
          ...
          <div id="member-pagination-wrap">${renderPagination(items.length, state.page, state.pageSize)}</div>
        </section>`;
