...
import { escapeHtml, renderDocument, renderLabelHtml, stat, dataTable, renderPagination } from "../shared/components.js?v=20260729";
...
function buildTable(items) {
  return dataTable(
    [
      { text: "NO", width: "64px" },
      { text: "KODE", width: "118px" },
      { text: "ANGGOTA", width: "190px" },
      { text: "BUKU", width: "280px" },
      { text: "PINJAM", width: "120px" },
      { text: "KEMBALI", width: "120px" },
      { text: "STATUS", width: "112px" }
    ],
    ...
    {
      emptyText: "Tidak ada data sirkulasi.",
      actionsWidth: "106px",
      actions: (row, index) => {
        const item = items[index];
        const hasActions = item.status === "dipinjam" || item.status === "terlambat";
      ...
      cellRenderer: (cell, colIndex) => {
        if (colIndex === 2) {
          const item = cell;
          return `<strong>${escapeHtml(item.member_name)}</strong><br><span style="color:#6e7979;font-size:12px">${escapeHtml(item.member_code)}</span>`;
        }
  ...
  const panelContent = state.loading
    ...
    : state.error
      ...
      : `<section class="panel" id="loan-panel">
          ...
          <div id="loan-pagination-wrap">${renderPagination(items.length, state.page, state.pageSize)}</div>
        </section>`;
